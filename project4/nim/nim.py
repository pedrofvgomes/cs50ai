...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in Nim.available_actions(state):
            q_values.append(self.get_q_value(tuple(state), tuple(action)))
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in Nim.available_actions(state):
            action_to_q_value[action] = self.get_q_value(tuple(state), tuple(action))
        
        if epsilon:
            if random.random() < self.epsilon:
                return random.choice(list(action_to_q_value.keys()))
            
        return next(action for action, q_value in action_to_q_value.items() if q_value == best_q_value)
